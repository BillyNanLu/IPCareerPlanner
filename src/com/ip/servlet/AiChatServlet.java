...
public class AiChatServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (userId == null) {
            // 未登录，跳转登录页面并带参数，登录后回跳 chat
            resp.sendRedirect("login.jsp?redirect=chat");
            return;
        }

        AiChatDao dao = new AiChatDao();
        List<AiChatMessageBean> history = dao.getMessagesByUserId(userId);
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        // 保存用户消息
        AiChatDao dao = new AiChatDao();
        AiChatMessageBean userMsg = new AiChatMessageBean();
        ...
        // 保存 AI 消息
        AiChatMessageBean aiMsg = new AiChatMessageBean();
    ...
    private String callDoubaoAI(String userInput, Integer userId) {
        try {
            ...
            AiChatDao dao = new AiChatDao();
            List<AiChatMessageBean> history = dao.getRecentMessages(userId, 5);

            // 构建 messages JSON
            JSONArray messagesArray = new JSONArray();
            for (AiChatMessageBean msg : history) {
