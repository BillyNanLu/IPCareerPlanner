...
<form action="${pageContext.request.contextPath}/courseEdit" method="post" enctype="multipart/form-data" class="space-y-6">
    <input type="hidden" name="id" value="${course.id != null ? course.id : 0}" />
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="number" step="0.01" name="price" value="${course.price}" class="w-full border rounded px-4 py-2" />
        </div>
        <div>
            <label class="block mb-1 font-medium">折扣（0~1）</label>
            <input type="number" step="0.01" name="discount" value="${course.discount}" class="w-full border rounded px-4 py-2" />
        </div>
    </div>
    ...
    <div>
        ...
        <textarea name="target_user" rows="3" class="w-full border rounded px-4 py-2">${course.targetUser}</textarea>
    </div>

    <!-- 推荐理由 -->
    <div>
        <label class="block mb-1 font-medium">推荐理由</label>
        <textarea name="recommendation" rows="3" class="w-full border rounded px-4 py-2">${course.recommendation}</textarea>
    </div>
